...
7. **Build Command:**
   ```bash
   npm install -g pnpm && pnpm install && pnpm --filter backend run db:generate && pnpm --filter backend run build
   ```
8. **Start Command:**
   ```bash
   pnpm --filter backend run db:deploy && pnpm --filter backend start
   ```
9. **Health Check Path:** `/health/ready`
...
7. **Build Command:**
   ```bash
   npm install -g pnpm && pnpm install && pnpm --filter frontend run build
   ```
8. **Start Command:**
   ```bash
